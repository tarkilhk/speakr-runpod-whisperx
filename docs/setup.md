...
| `RUNPOD_STUCK_INIT_TIMEOUT_SECONDS` | `300` | Template mode: **`machineId`** set, **`runtime`** still missing, warmup **fingerprint** unchanged this long ⇒ terminate and redeploy. GraphQL cannot distinguish image pull vs deadlock; use a generous value. `0` disables. |
